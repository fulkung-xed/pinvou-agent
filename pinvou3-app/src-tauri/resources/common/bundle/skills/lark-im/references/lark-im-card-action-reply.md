...
> **Prerequisite:** `event consume` essentials live in the `lark-event` skill, which is not bundled in this package. Use the CLI directly: run `lark-cli event --help` / `lark-cli event consume --help` for the real flags before consuming events.
