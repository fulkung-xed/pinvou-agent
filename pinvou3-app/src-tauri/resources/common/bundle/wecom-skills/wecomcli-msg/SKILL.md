---
name: wecomcli-msg
description: 何时用:仅当用户明确指向企业微信消息(看企微聊天记录、发企微消息)时使用;泛指发消息/看通知默认走本地渠道,不要误用。企微消息:会话列表查询、消息记录拉取(支持文本/图片/文件/语音/视频)、多媒体文件获取和文本消息发送。用户需要"查看消息"、"看聊天记录"、"发消息给某人"、"最近有什么消息"、"给群里发消息"、"看看发了什么图片/文件"时使用。
metadata:
  requires:
    bins: ["wecom-cli"]
